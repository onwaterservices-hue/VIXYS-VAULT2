...
export async function executeAutoTradesForSignal(
  ...
  const executionPromises = enabledUsers.map(async (userState) => {
    ...
    // PATCH 2: MULTI-COIN AUTO-TRADING SAFETY
    // The upstream active15mCycle is BTC-centric. Other assets are not independently supported for execution.
    if (asset !== 'BTC') {
    ...
    // PATCH 1: NATIVE SIGNAL EXPIRATION GUARD
    // Enforce that the canonical signal is still valid using server time immediately before proceeding.
    if (signal.expiresAt) {
