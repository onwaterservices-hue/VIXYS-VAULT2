...
export const ScalpDecisionChart: React.FC<ScalpDecisionChartProps> = ({
  ...
  useEffect(() => {
    ...
    fetchCryptoTicker(asset)
      .then((data) => {
        ...
        setCandles(createRealisticCandles(p, 30, candleInterval, candleStepScale));
      })
      .catch(() => {
        if (!isCancelled) {
          setCurrentPrice(64160.50);
          setStrikePrice(64150.50);
          setCandles(createRealisticCandles(64160.50, 30, candleInterval, candleStepScale));
        }
      });
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const effectiveCurrentPrice = (currentPrice > 100 && Math.abs(currentPrice - lastClose) / lastClose < 0.05) ? currentPrice : lastClose;

      if (effectiveCurrentPrice > 100) {
        rawMinP = Math.min(rawMinP, effectiveCurrentPrice);
        rawMaxP = Math.max(rawMaxP, effectiveCurrentPrice);
      }
