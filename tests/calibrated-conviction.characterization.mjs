...
t.section('every surface reads the SAME headline (P(win) else ENGINE SCORE, never a default)');
const strip = (s) => s.replace(/\/\*[\s\S]*?\*\//g, '').split('\n').filter((l) => !l.trim().startsWith('//')).join('\n');
const sem = strip(readRepoFile('src/lib/engineSemantics.ts'));
const rail = strip(readRepoFile('src/components/vixyV2/ContextualRightRail.tsx'));
const ring = strip(readRepoFile('src/components/CycleObject.tsx'));
const hub = strip(readRepoFile('src/components/VixyHubView.tsx'));
const heroSrc = strip(readRepoFile('src/components/CryptoPredictionCenterView.tsx'));
...
t.check('the decision type declares calibrated and marketRead (no more `as any` reads on new surfaces)', readRepoFile('src/types/canonicalDecision.ts').includes('calibrated?: CalibratedConviction | null;') && readRepoFile('src/types/canonicalDecision.ts').includes('marketRead?: {'));
t.done();
