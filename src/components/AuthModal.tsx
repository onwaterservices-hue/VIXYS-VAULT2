  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeof handleSuccessCB === 'function') {
        setTimeout(() => handleSuccessCB(finalRole as any, userEmail, finalProduct, mode === 'register'), 150);
      }
      setLoading(false);
      onClose();
    } catch (err) {
    ...
    >
      <div className="relative w-full max-w-lg bg-gradient-to-b from-[#130B2A] via-[#0A0518] to-[#070312] border-2 border-purple-500/50 rounded-3xl shadow-2xl shadow-purple-950/90 overflow-hidden text-purple-100 my-auto">
        ...
        <div className="p-6 sm:p-8 space-y-6">
          {/* Top Mode Switcher Tabs - ONLY Sign In & Create Account */}
          ...
          <div className="text-center space-y-2">
            ...
            <h2 className="text-2xl sm:text-3xl font-black font-sans tracking-tight text-white">
              {mode === 'login' ? 'Sign In to Terminal' : 'Create Your VIXY Account'}
            </h2>
          ...
          <div className="text-center pt-1">
            {mode === 'login' ? (
              <button
                type="button"
                onClick={() => { setMode('register'); setErrorMsg(''); setSuccessMsg(''); }}
                className="text-xs text-purple-300/70 hover:text-cyan-300 transition-colors font-sans"
              >
                New to VIXY? <span className="font-bold text-cyan-400 underline">Create Account</span>
              </button>
            ) : (
              <button
                type="button"
                onClick={() => { setMode('login'); setErrorMsg(''); setSuccessMsg(''); }}
                className="text-xs text-purple-300/70 hover:text-purple-200 transition-colors font-sans"
              >
                Already have an account? <span className="font-bold text-purple-300 underline">Sign In</span>
              </button>
            )}
          </div>
