...
const live = {
  ...
  orderFlowAnalytics: { takerBuyRatio: 1 },
...
t.eq('neutral flow proxy scores 5.0 (was 7.0)', L['Order Flow'].score, 5);
t.eq('...and is not counted as support (was aligned)', L['Order Flow'].aligned, false);
t.check('flow detail names a price-derived proxy, not a taker ratio',
  /proxy/.test(L['Order Flow'].detail) && /no trade tape/.test(L['Order Flow'].detail) && !/Taker buy ratio/.test(L['Order Flow'].detail));
...
const bull = { ...live, orderFlowAnalytics: { takerBuyRatio: 1.5 }, priceStructure: { displacementUSD: 40, breakoutState: 'BREAKOUT_BULL' },
...
const bear = { ...live, orderFlowAnalytics: { takerBuyRatio: 1 / 1.5 }, priceStructure: { displacementUSD: -40, breakoutState: 'BREAKOUT_BEAR' },
...
t.check('both name the spot-vs-strike proxy',
  serverSrc.includes('Spot-vs-strike flow proxy (no trade tape)') && serverSrc.includes('Spot-vs-strike flow proxy leans'));
...
t.eq('...and the neutral flow proxy is not aligned', withLive['Order Flow'].aligned, false);
